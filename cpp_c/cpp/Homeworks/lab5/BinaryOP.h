...
//X^a == power(x,a)
class Power final :public BinaryOP{
    public:
    Power(BaseExpr *left, BaseExpr *right) : BinaryOP(left, right){}
    ~Power() final = default;
    double evaluate() const override;
    double evaluate(double x) const override;
    BaseExpr* getDerivative() const override;
...
class Log final : public BinaryOP{
    public:
    Log(BaseExpr *left, BaseExpr* right): BinaryOP(left,right){};
    ~Log() final = default;
    double evaluate() const override;
    double evaluate(double x) const override;
    BaseExpr* getDerivative()const override;
