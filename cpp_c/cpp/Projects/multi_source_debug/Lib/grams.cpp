...
void filter_words(vector<pair<string, int>>& freq, vector<string>& words) {
    ...
    record_freq(words,freq);
    //sort the vector
    sort(freq.begin(),freq.end(),[](pair<string,int> x , pair<string,int>y)-> bool {return x.second>y.second;});
...
void set_coocur_matrix(const vector<string>& words, const map<string, int>& word_index, vector<vector<int>>& coocur_matrix, int n) {
    ...
    coocur_matrix[(word_index.find(words[center]))->second][(word_index.find(words[left]))->second]++;//add the left 
    coocur_matrix[(word_index.find(words[center]))->second][(word_index.find(words[right]))->second]++;//add the right
...
void normalize_matrix(const vector<vector<int>>& coocur_matrix, vector<vector<double>>& normalized_matrix) {
    size_t size=coocur_matrix.size();
    size_t i=0;
    size_t sze_int=coocur_matrix[0].size();
    size_t j=0;//set the i,j for coordination
    ...
    {   
        ...
        while(i<size)
        {
            ...
            i++;
        }//calculate each sum of the matrix
        i=0;
        while(i<size)
        {
            normalized_matrix [i][j]=(coocur_matrix[i][j]*1.0)/(sum*1.0);
            i++;
        }
        j++;
...
void save_matrix(const string& filename, const vector<vector<double>>& matrix, const map<string, int>& word_index) {
    ...
    while(iter!=word_index.end())
        file<<iter->first<<' ';//output the words
    file<<endl;
