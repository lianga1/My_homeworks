...
int KMP(string *ori_str,string *mod_str,int i);//use KMP get the position of the understring
...
int main()
{
    ...
    for(i = 0;i<ori_str->lenth;i++)
    {
        i=KMP(ori_str,mod_str,i);
        if(i!=-1)//use KMP to find the understring
            result++;
        if(i==-1)
            break;
    }
...
void Get_Next(string *str,int next[])//get the next array of the string
{
    ...
    */
    ...
    while(j<str->lenth-1)
    {
        if(k==-1||(str->data[k]==str->data[j]||str->data[k]=='*'||str->data[j]=='*'))//when return to the next[0] or start of the scan or the matching is continueing
        {
...
void Get_Next_I(string *str,int next_i[])
{
    ...
    while(j<str->lenth)
    {
        if(k==-1||(str->data[k]==str->data[j]||str->data[k]=='*'||str->data[j]=='*'))
        {
            j++;k++;
            if((str->data[k]!=str->data[j])&&(str->data[k]!='*')&&(str->data[j]!='*'))
                next_i[j]=k;
            else 
                next_i[j]=next_i[k];
        }
...
int KMP(string *ori_str,string *mod_str,int k)//use KMP get the position of the understring
{
    int next[MaxSize];
    int i=k,j=0;

    Get_Next(mod_str,next);//get the next_Array of the mod string
    ...
    {
        if(ori_str->data[i]=='*'||mod_str->data[j]=='*'||ori_str->data[i]==mod_str->data[j]||j==-1)//if the current char is matched , or when return to the first
        {
